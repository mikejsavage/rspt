...
fn pixels_for_thread( thread : usize ) -> Vec< usize > {
	let mut ps = Vec::new();
	let mut p = thread;
...
fn irradiance( world : &World, start : Vec3, dir : Vec3, depth : usize ) -> RGB {
	if depth > 5 {
		return RGB::black();
	}
...
fn sampler( eye : Vec3, pixels : &[ usize ], centres : &[ Vec3 ], world : &World, up_pixel : Vec3, left_pixel : Vec3 ) {
	let mut samples = 0;
	...
	loop {
		...
		for i in range( 0, pixels.len() ) {
			...
			unsafe {
				let p = pixels[ i ];
				image[ p ] = ( samples, image[ p ].1 + color );
			}
		}
